...
function ImageModal({ product, baseUrl, onClose, onRefresh, setGlobalMsg }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      for (let i = 0; i < previews.length; i++) {
        ...
        await ax.post(`/sanpham/${product.sanPhamId}/hinhanh`, fd, {
      ...
      setGlobalMsg({
        type: "success",
        text: `Đã upload ${previews.length} ảnh thành công`,
      });
    } catch {
  ...
  const handleSetMain = async (imgId) => {
    try {
      await ax.patch(`/sanpham/${product.sanPhamId}/hinhanh/${imgId}/setMain`);
      await onRefresh(product.sanPhamId);
    } catch {
  ...
  const handleDeleteImage = async (imgId) => {
    try {
      await ax.delete(`/sanpham/${product.sanPhamId}/hinhanh/${imgId}`);
          ...
          <div className="modal-header">
            <div>
              ...
              <p>
                {product.tenSanPham}
                <span
                  style={{ marginLeft: 8, fontWeight: 600, color: "#2563eb" }}
                >
                  {images.length} ảnh
                </span>
              </p>
          ...
          <div className="img-modal-body">
            ...
            {images.length > 0 ? (
              <div className="img-gallery">
                {images.map((img, i) => (
                  ...
                  >
                    ...
                    {/* Badges */}
                    {img.isMain === 1 && (
                      <span className="img-main-badge">✓ Chính</span>
                    )}
                    ...
                    {deleteTarget === img.id && (
                      <div
                        className="img-delete-confirm"
                        onClick={(e) => e.stopPropagation()}
                      >
                        <p>Xóa ảnh này?</p>
                        <div className="img-delete-confirm-btns">
                          <button
                            className="img-confirm-yes"
                            onClick={() => handleDeleteImage(img.id)}
                          >
                            Xóa
                          </button>
                          <button
                            className="img-confirm-no"
              ...
              <div className="img-empty">
                ...
                <p className="img-empty-text">
                  Sản phẩm chưa có hình ảnh nào.
                  <br />
                  Thêm ảnh bên dưới để hiển thị.
                </p>
              </div>
            )}
            ...
            <div className="img-upload-zone">
              ...
              >
                ...
                <p className="img-drop-text">
                  Kéo thả ảnh vào đây hoặc <strong>chọn file</strong>
                </p>
                <p className="img-drop-hint">
                  JPG, PNG, WEBP — nhiều ảnh cùng lúc
                </p>
              </div>
            </div>
...
function ProductDetailModal({
  ...
  const catName = (id) =>
    categories.find((c) => String(c.id) === String(id))?.tenLoai ??
    `Loại ${id}`;
            ...
            <div className="detail-main-img-wrap">
              ...
              {images.length > 1 && (
                <>
                  ...
                  <span className="detail-img-counter">
          ...
          <div className="detail-info-col">
            ...
            >
              <span className="badge badge-cat">
                {catName(product.loaiSanPham)}
              </span>
              <span
                className={`badge ${product.trangThai === 1 ? "badge-active" : "badge-inactive"}`}
              >
            ...
            <div className="detail-info-table">
              ...
              <div className="detail-info-row">
                ...
                <span
                  className={product.slTon < 10 ? "stock-low" : "stock-ok"}
...
export default function SanPhamManager() {
  ...
  const fetchAll = async () => {
    try {
      ...
      setMessage({
        type: "error",
        text: "Không thể tải dữ liệu. Kiểm tra kết nối backend.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const refreshProduct = useCallback(async (sanPhamId) => {
    ...
    setImgProduct((prev) =>
      prev ? (res.data.find((p) => p.sanPhamId === sanPhamId) ?? prev) : null,
    );
  }, []);
  ...
  const filtered = useMemo(
    () =>
      products.filter((p) => {
        ...
        const matchSearch =
          !kw ||
          p.tenSanPham?.toLowerCase().includes(kw) ||
          p.maSanPham?.toLowerCase().includes(kw);
        const matchCat = !filterCat || String(p.loaiSanPham) === filterCat;
        const matchStatus =
          filterStatus === "" || String(p.trangThai) === filterStatus;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      editId
        ? await ax.put(`/sanpham/${editId}`, payload)
        : await ax.post("/sanpham", payload);
      await fetchAll();
      resetForm();
      setMessage({
        type: "success",
        text: editId ? "Cập nhật thành công" : "Thêm sản phẩm thành công",
      });
    } catch {
      setMessage({
        type: "error",
        text: "Lưu thất bại. Kiểm tra lại dữ liệu.",
      });
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async (p) => {
    try {
      await ax.delete(`/sanpham/${p.sanPhamId}`);
      ...
      <section className="hopdong-header">
        <div>
          ...
          <p className="subtitle">
            Thêm, cập nhật và quản lý hình ảnh cho từng sản phẩm.
          </p>
        </div>
        <div className="toolbar">
          ...
          />
          <select
            className="search sanpham-filter"
            value={filterCat}
        ...
        <div className="panel table-panel">
          <div className="panel-head">
            <div>
              ...
              <p>
                Hiển thị {filtered.length}/{products.length} bản ghi.
              </p>
            </div>
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  filtered.map((p) => (
                    <tr key={p.sanPhamId}>
                      <td>
                        <span className="product-code">{p.maSanPham}</span>
                      </td>
                      ...
                      <td
                        className={`right ${p.slTon < 10 ? "stock-low" : "stock-ok"}`}
                      >
                        {p.slTon}
                      </td>
                      <td>
                        <span
                          className={`badge ${p.trangThai === 1 ? "badge-active" : "badge-inactive"}`}
                        >
