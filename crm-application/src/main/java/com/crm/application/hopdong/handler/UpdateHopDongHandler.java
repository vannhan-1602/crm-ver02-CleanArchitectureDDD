...
@Service
public class UpdateHopDongHandler {
    ...
    public HopDong handle(Long id, UpdateHopDongCommand command) {
        HopDong existing = hopDongRepo.findById(id)
                .orElseThrow(() -> new NoSuchElementException("Hợp đồng không tồn tại: " + id));
